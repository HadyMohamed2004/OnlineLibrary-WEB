...
// Predefined books to be added
const predefinedBooks = [
    {
        id: 7,
        title: "1984",
        author: "George Orwell",
        cover: "https://covers.openlibrary.org/b/id/8575691-L.jpg",
        category: "Classics",
    ...
    },
    {
        ...
        cover: "https://covers.openlibrary.org/b/id/12003633-L.jpg",
        category: "Fantasy",
...
// Function to load predefined books into localStorage
function loadPredefinedBooks() {
    let existingBooks = JSON.parse(localStorage.getItem('books')) || [];
    
    // Check if predefined books are already added by looking for one of their IDs
    const predefinedBookExists = existingBooks.some(book => predefinedBooks.some(pb => pb.id === book.id));
    
    if (!predefinedBookExists) {
        // Add predefined books to existing books
        const updatedBooks = [...existingBooks, ...predefinedBooks];
...
function initializeFilters() {
    ...
    categories.forEach(category => {
        categoryFilter.innerHTML += `<option value="${category.toLowerCase()}">${category}</option>`;
    });
}